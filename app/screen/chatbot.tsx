...
export default function ChatbotScreen({
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await fetch(`${API_URL}/bisabot/history`);
  ...
  const handleClearHistory = async () => {
    try {
      await fetch(`${API_URL}/bisabot/history`, { method: "DELETE" });
      setMessages([
        {
          id: Date.now().toString(),
          text: `Selamat Siang Denai. Ini adalah AI Insurance Guide. SIlahkan bertanya terkait pembuatan asuransi.`,
          sender: "assistant",
        },
      ]); // kosongkan di UI
    } catch (err) {
